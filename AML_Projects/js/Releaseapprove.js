...
function GetType() {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "Releaseapprove.aspx/Gettype",
...
function GetReleaseCategory() {
    ...
    $.ajax({
        ...
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: "Releaseapprove.aspx/GetReleaseCategory",
        data: "{QueryString:'" + QueryString + "'}",
        dataType: "json",
        success: function (Result) {
            $('#ddlcategory').empty();
...
function releaseapproveconfirm() {
    var approve;
    var QueryString = "CONFIRMAPPROVE";
    if ($("#radAppr").prop("checked") == true) {
        approve = 7;

    }
    else if ($("#radRjct").prop("checked") == true) {
        approve = 8;
    ...
    if (approve== "") {
        ModelPopWarning("Please Select Approve Status");
        //alert("Please Select Financial Type");
        return false;
    }
  
    else if (tblAmntData == "") {
        ModelPopWarning("Please Select Data to Approve");
        //alert("Please Select Financial Type");
        return false;
    }
        ...
        var input = approve  ;
                    $.ajax({
                        type: "POST",
                        contentType: "application/json; charset=utf-8",
                        url: "Releaseapprove.aspx/confirmrelease",
                        data: "{QueryString:'" + QueryString + "',input : '" + input + "',arry:'" + tblAmntData + "'}",
                        dataType: "json",
                        success: function (Result) {
                            //alert(Result.d);
                            Result = Result.d;
                            ...
                            window.open('Releaseapprove.aspx', '_self');
...
function getrrapprovaltable() {
    ...
    Querystring = "GETRRDATA";
        ...
        $.ajax({
            type: "POST",
            contentType: "application/json; charset=utf-8",
            url: "Releaseapprove.aspx/getTblchnge",
            ...
            success: function (Result) {
                if (Result.d.length > 0) {
                    ...
                    $("#tablech").append('<thead class="bg-success text-white">< tr ><th scope="col">SR ID</th><th scope="col">Reported Date</th><th scope="col">CRF ID</th><th scope="col">Parent App</th><th scope="col">Summary of change</th><th scope="col">Developer</th> <th scope="col">Tester</th><th scope="col">Code review</th></tr></thead><tbody class="border border-dark">'
                    );
                    for (var i = 0; i < Result.d.length; i++) {
                        $("#tablech").append('<tr><td>' + Result.d[i].rrid + '</td>' +
                           
                            '<td>' + Result.d[i].date + '</td>' +
                          
                            '<td>' + Result.d[i].changeid + '</td>' +
                            '<td>' + Result.d[i].crfid + '</td>' +
                            '<td>' + Result.d[i].parent + '</td>' +
                            '<td>' + Result.d[i].summary + '</td>' +
                            '<td>' + Result.d[i].developer + '</td>' +
                            '<td>' + Result.d[i].tester + '</td>' +
                            

                            '<td><input class="form-control input-sm align-right " id="chkbx' + (i + 1) + '"  name="Pay" type="checkbox" onclick="addIntrst(' + i + ')"/></td>' +
...
function addIntrst(i) {
    ...
    var table = document.getElementById("tablech");
    var l = $("#tablech tr").length;
    for (i = 1; i < l; i++) {

        if ($('#chkbx' + i).prop('checked')) {
            tblAmntData = tblAmntData + table.rows[i].cells[0].innerText + "¥";
...
//        type: "POST",
//        contentType: "application/json; charset=utf-8",
//        url: "Releaseapprove.aspx/getlinkdata",
//        data: "{QueryString:'" + QueryString + "',input : '" + input + "'}",
//        dataType: "json",
//        success: function (Result) {
