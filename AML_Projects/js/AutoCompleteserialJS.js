    ...
    if (arr.length > 0) {
        ...
        for (i = 0; i < arr.length; i++) {
            ...
            b.addEventListener("click", function (e) {
                //var k = this.getElementsByTagName("input")[0].value;
                //$("#txt_emp_cod").text(k);
                    /*insert the value for the autocomplete text field:*/
                    document.getElementById('' + inp + '').value = this.getElementsByTagName("input")[1].value;
                document.getElementById('txt_sernum_mov').value = this.getElementsByTagName("input")[0].value;
               ...
               // Setval(this.getElementsByTagName("input")[0].value);
              //  $("[id*=selempcode]").val(this.getElementsByTagName("input")[0].value);
                //document.getElementById('' + txt_emp_cod + '').innerHTML = this.getElementsByTagName("input")[0].value;
               // alert(this.getElementsByTagName("input")[0].value);
               // $('#txt_emp_cod').val(this.getElementsByTagName("input")[0].value);
                    /*close the list of autocompleted values,
                    (or any other open lists of autocompleted values:*/
                closeAllLists();
                
               //alert $("[id*=selempcode]").val();
                });
                a.appendChild(b);
            //}
        }
...
function LoadDataAutoComplete(ControlID, SearchKey, SrchStringLen, hiddenVariable, QueryFlag, QueryID, ShowValue) {
    ...
    if (SearchKey.length >= SrchStringLen) {
        $.ajax({
            ...
            url: "ServiceAutoComplete.asmx/GetSearchData",
...
function LoadingDataserail(ControlID, SearchKey, SrchStringLen, hiddenVariable, QueryFlag, QueryID, ShowValue) {
    //if ($('#chk_tickno_mov').prop('checked') == true || $('#chk_rpt_dcmn').prop('checked') == true || $('#chk_rpt_dsps').prop('checked') == true)
    //{
        var ArrayList = [];
        if (SearchKey.length >= SrchStringLen) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "ServiceAutoComplete.asmx/GetSearchingData",
                data: "{SearchKey:'" + SearchKey + "',QueryFlag:'" + QueryFlag + "',QueryID:'" + QueryID + "'}",
                dataType: "json",
                success: function (Result) {
                    ArrayList = Result.d;
